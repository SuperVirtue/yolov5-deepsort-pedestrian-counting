...
def draw_boxes(img, bbox, identities=None, offset=(0, 0)):
    for i, box in enumerate(bbox):
        ...
        cv2.rectangle(img, (x1, y1), (x2, y2), color, 1)  #绘制标注人体的框
        cv2.rectangle(img, (x1-4, y1), (x1 + t_size[0]-20, y1 + t_size[1]+4), color, -1) #绘制标注ID的框
        cv2.putText(img, label, (x1, y1 + t_size[1] + 4), cv2.FONT_HERSHEY_PLAIN, 1.3, [0, 0, 0], 1)
    return img
